...
async function loadDashboard() {
  // [PATCH_UI_MARKET_REGIME_V1] - charge le panneau Regime Marche
...
function renderPendingValidationPanel(orders) {
  // [FIX_OPTION_2_UNIFIED_QUEUE_V1] : disabled, replaced by Pending Approvals card
  const panel = document.getElementById('pendingValidationPanel');
  if (!panel) return;
  panel.innerHTML = '';
  return;
...
// [PATCH_UI_MARKET_REGIME_V1] --- helpers Regime Marche ---
...
async function loadMarketRegimeCard() {
  ...
  try {
    ...
    document.getElementById('mr-impact').textContent = 'Erreur chargement r\u00e9gime : ' + (err.message || err);
    return;
  }
  if (!data || data.error) {
    document.getElementById('mr-impact').textContent = 'Donn\u00e9es indisponibles' + (data && data.error ? ' : ' + data.error : '');
    return;
  }
  const cycleEl = document.getElementById('market-regime-cycle');
  cycleEl.textContent = 'Cycle ' + (data.cycle_id || '\u2014') + '  -  Portfolio: ' + (data.portfolio_regime || '\u2014');
  ...
  function _impactFor(label, b) {
    if (!b) return null;
    if (b.regime === 'STRESS') return label + ' STRESS : BUY x' + b.buy_mult + ', SELL x' + b.sell_mult + ' (acheter la baisse)';
    if (b.regime === 'CALM')   return label + ' CALM : BUY x' + b.buy_mult + ', SELL x' + b.sell_mult + ' (take profit facilit\u00e9)';
...
/* [SHADOW_UI_V1] BEGIN */
/* [SHADOW_UI_V1_FIX_CACHE_KEY] */ /* 20260612_184147 */
/* [SHADOW_UI_V1_POLISH] */
...
(function(){
  ...
  async function renderShadowVariants(){
    ...
    try {
      ...
      var html = "";
      rows.forEach(function(r){
        shadowRowsCache[r.variant_id] = r;
        ...
        html += '<td style="padding:6px 8px;text-align:center;">'+recoBadge(r.recommendation, r.variant_id, hasMemo)+'</td>';
        html += '</tr>';
      });
      tbody.innerHTML = html;
      cleanupParasites();
